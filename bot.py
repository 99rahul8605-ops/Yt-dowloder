...
YT_DLP_OPTIONS = {
    ...
    # Ensure we use a recent yt-dlp that handles modern YouTube
    "user_agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
    "extractor_args": {"youtube": {"player_client": ["android", "web"]}}  # try both clients
}
...
def extract_url(text: str) -> str | None:
    ...
    # Simple regex – covers most YouTube URLs
...
def progress_hook(queue, loop):
    ...
    def hook(d):
        if d['status'] == 'downloading':
            # Put the progress dict into the queue safely from a thread
...
async def progress_updater(queue, message):
    ...
    while True:
        try:
            # Wait for a progress update, timeout to allow cancellation
            ...
            # Edit message at most every 2 seconds
            ...
            # No update for 2 seconds – download probably finished
            continue
        except asyncio.CancelledError:
            # Updater task cancelled – download done
            break
...
async def download_and_upload(url: str, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # We'll try best video+audio first; if it exceeds limit, we fallback to audio only.
        # For a more accurate decision, we could parse formats, but this keeps it simple.
        ...
        try:
            # Run download in thread pool (blocking)
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # Cancel progress updater (download finished)
        ...
        if not video_downloaded:
            ...
            ydl_opts.update({
                ...
                "outtmpl": os.path.join(tmpdir, "%(title)s.%(ext)s"),
                "postprocessors": [{  # Convert to mp3 for maximum compatibility
            ...
            for attempt in range(retries + 1):
                try:
                    ...
                    # Find the audio file (should be .mp3)
                    files = list(Path(tmpdir).glob("*.mp3"))
                    if not files:
                        # Maybe the file is not mp3 (e.g., if ffmpeg missing)
                        files = list(Path(tmpdir).glob("*"))
                    if not files:
